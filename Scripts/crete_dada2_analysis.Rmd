...
# require(stringr)
# 
# seqtab.nochim<-readRDS("all_runs_dada2_abundance_table.rds")
...
# 
# #taxa<-readRDS("taxa_17Nov2022.RDS")
...
# #write.csv(seq, "Crete_ASV_countTable_silva_nr99_v138.csv", quote=F, row.names=F)
```


#Read in data
```{r}
setwd("~/Dropbox (IGS)/Crete/")
count<-read.csv("Crete_ASV_countTable_silva_nr99_v138.csv", header=TRUE)
#count<-seq
rownames(count)<-count$sampleID
count$sampleID<-NULL
count[is.na(count)]<-0

meta<-read.csv("Composite_MetaData_from_master.csv", header=TRUE)
...
tiff("Figure_2.tiff", height = 2400, width=4200, res=600)
bottom_row
dev.off()

png("Figure_2.png", height = 2400, width=4200, res=600)
bottom_row
dev.off()
```
...
ggplot(t[1:100, ], aes(x=reorder(taxa, -as.numeric(cumsum)), y=as.numeric(cumsum)))+geom_bar(stat="identity")+theme(axis.text.x = element_text(angle=90, hjust=1, size=4))
dev.off()
...
ggplot(the.clusts.m, aes(x=value, y=PcOA_value))+
  ...
  stat_cor(aes(label = paste(..rr.label.., ..p.label.., sep = "~")), 
           ...
           geom = "text", 
           size=2)
ggsave("Figure_3.png", height=4, width=9)
ggsave("Figure_3.tiff", height=4, width=9)
```
...
colfunc <- colorRampPalette(c("khaki", "limegreen", "darkslategray1", "mediumblue", "magenta", "red"))
require(pheatmap)
png("Figure_4_average.png", width=7, height=6, units="in", res=600)
...
ggsave("Figure_5.tiff", height=3, width=10)
ggsave("Figure_5.png", height=3, width=10)

the.fit.m3$Feature<-gsub("Moisture\n", "Moisture ", the.fit.m3$Feature)
write.table(the.fit.m3, "Table_S1.txt", quote=F, row.names = F, sep="\t", eol = "\r")
```
